...
また, Color Roleをtokenとして扱うことで, デザインシステムをより効果的に管理することができる.
...
- Primary, Secondary, Tertiary: 強調や強調しないforeground要素に使われるアクセントカラーに使われるrole.
- Container: ボタンのようなforeground要素の塗りつぶし色として使われるrole. textやiconには使われない.
- On: この単語で始まるroleはtextやiconの色を対となる親の色の上に置くことを示す. 例えば, `on primary`はprimary塗りつぶし色に対するtextやiconに使用される.
...
で構成されるbuttonはコントラストレベルの可視性を保つために使われる.
対して,
...
より自然なbackgroundにはsurface roleを使い, カードのようなコンポーネントにはcontainer roleを使う.

surface には3つのroleがある.

- surface: 背景のデフォルト色
- on surface: 任意のsurfaceに対するtextやiconの色
- on surface variant: 任意のsurfaceに対するtextやiconの控えめな強調色
...
以上はlight theme でのsurfaceとon surface
...
以下はlight theme と dark themeにおける5つのsurface contaienr roleの例
...
surface roleの最も一般的な使い方として背景としてsurfaceを使い, navigation areaにsurface containerを使うというものがある.
...
例えばモバイルとタブレットでも, bodyエリアはsurfaceを, navエリアはsurface containerを使うべきである.
...
この例では, bodyとnavの領域が画面サイズクラス(surface と surface containerそれぞれ)を通して同じcolor roleを持っていて, 巨大な画面サイズに他のsurface containerの変化バージョンを追加している.
...
コンポーネントに適用されるデフォルトのsurface container role
...
逆のrolesはコンポーネントに対して選択的に適用され, 周囲のUIのと逆の色を構成し, コントラスト効果を生み出す.

- Inverse surface: surfaceに対しコントラストになる要素の背景の塗りつぶし
- Inverse on surface: inverse surfaceに対するtextやiconの色
...
light theme でのcolor schemeにおけるinverse surface, inverse on surface, inverse primary role
...
- (2) textに対するInverse on surface
- (3) text buttonに対するInverse primary

### Primary (原色, 1次色)

- Primary: 強調的な塗りつぶし, surfaceに対するtext, icon.
- On primary: primaryに対するtextやiconの色.
- Primary container: surfaceに対し独立した塗りつぶし色, FAB(❤)のようなkey要素に使われる.
- On Primary container: primary containerに対するtextやiconの色.
...
- Secondary: surfaceに対する目立ちにくい塗りつぶしtext, icon
- On secondary: secondaryに対するtextやiconの色
- Secondary container: トーンボタンのような劣性のコンポーネントのsurfaceに対するあまり目立たない塗りつぶし色
- On secondary container: secondary containerに対するtextやiconの色
...
Secondary color roleはsecondaryとsecondary containerとそれらに対する"on" colorを含む.
...
- Teritary: surfaceの目立つ塗りつぶしtext, iconの補助
- On tertiary: tertiaryに対するtextやiconの色
- Tertiary container: 入力フィールドのようなコンポーネントのsurfaceに対する補完的なコンテナの色.
- On tertiary container: tertiary containerに対するtextやiconの色
...
- Error: 塗りつぶしicon, text, 緊急性に関するsurfaceに対する注意を引くための色
- On error: errorに対するtextやiconの色
- Error container: surfaceに対する注意を引く塗りつぶし色
- On error container: error containerに対するtextやiconの色
