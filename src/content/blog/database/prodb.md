...
そのため, DB設計はシステムの"設計"の最初に行うべきである(といのが著者の主張).
...
本来は多重度(もしくはカーディナリ)と呼ばれるn:mの関係を表すための記号があるが, ここでは省略する.
...
1. 大規模なテーブルの属性(logNの計算量を活かすため, Nが小さいと線形探索の方が速い)
1. 多重度の高い属性(参照に使われることが多い属性)
1. WHERE句やJOIN句の条件に使われる属性

まず, logNの計算量を活かすために大規模なテーブルの属性に適用するのが原則となる. 逆にNが小さい場合は線形探索の方が速いため, インデックスを使うのは逆効果である.
